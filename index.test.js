...
let messages = [];
const logger = { log: (m) => messages.push(m), group: (m) => messages.push(m), groupEnd: () => { } }
...
describe("eslint-ratchet", () => {
    ...
    it("throws errors and messages when violations increase", () => {
        ...
        const expectedMessages = [
            ...
            '🔥',
            'These latest eslint results have been saved to \x1B[33m\x1B[4meslint-ratchet-latest.json\x1B[24m\x1B[39m. \n' +
        ...
        const newValues = JSON.parse(fs.readFileSync("./eslint-ratchet-latest.json"));
...
const setupMocks = () => {
    mock({
        'eslint-ratchet.json': mock.file({ content: JSON.stringify(getMockThresholds())}),
        'eslint-ratchet-latest.json': JSON.stringify({}),
    });
};
...
const getMockResults = () => [
    ...
    {
        "filePath": "app/javascript/components/integrations/saved-import-queries/saved-import-query-item.jsx",
        "messages": [
            {
                "ruleId": "react/jsx-no-target-blank",
                "severity": 2,
    ...
    {
        "filePath": "app/javascript/components/list-view/dnd-handler/drop-handlers/tags/cell-to-cell-tag-drop-handler.js",
        "messages": [
            {
