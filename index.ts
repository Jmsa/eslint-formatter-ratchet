...
const logColorfulValue = (
  ...
  logger.log(
    `--> ${violationType}: ${colorFn(value)} (previously: ${chalk.yellow(
      previously,
    )})`,
  );
};
...
const detectAndLogChanges = (
  ...
  // issue have gone up
  let newIssues = 0;
  const updatedResults: FileIssues = structuredClone(previousResults);
      ...
      Object.entries(fileValue).forEach(([rule, result]) => {
        ...
        if (!result) {
          ...
          Object.entries(result).forEach(([violationType, value]) => {
            ...
            const previousValue = previousRule[violationType] ?? 0;
...
const formatter = function (
  ...
  let previousIssues: FileIssues = {};
  if (fs.existsSync("./eslint-ratchet.json")) {
    previousIssues = JSON.parse(
      fs.readFileSync("./eslint-ratchet.json", "utf8"),
    );
  }
  ...
  results.forEach(({ messages, filePath, errorCount, warningCount }) => {
    ...
    if (errorCount > 0 || warningCount > 0) {
      ...
      messages.forEach(({ ruleId, severity }) => {
        ...
        latestIssues[file][ruleId].warning =
          (latestIssues[file][ruleId].warning ?? 0) + (severity === 1 ? 1 : 0);
        latestIssues[file][ruleId].error =
          (latestIssues[file][ruleId].error ?? 0) + (severity === 2 ? 1 : 0);
      });
    }
  });
  ...
  const addsMatchingLintedFiles = Object.entries(added).filter(([k]) =>
    filesLinted.includes(k),
  );
  const updatesMatchingLintedFiles = Object.entries(updated).filter(([k]) =>
    filesLinted.includes(k),
  );
  const deletesMatchingLintedFiles = Object.entries(deleted).filter(([k]) =>
    filesLinted.includes(k),
  );
